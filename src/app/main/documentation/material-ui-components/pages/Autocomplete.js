...
function AutocompleteDoc(props)
{
    ...
    return (
        ...
        <>
            ...
            <ol>
                <li>The value for the textbox must be chosen from a predefined set of allowed values, e.g., a location field must contain a valid location name: <a href="#combo-box">combo box</a>.
                </li>
                <li>The textbox may contain any arbitrary value, but it is advantageous to suggest possible values to the user, e.g., a search field may suggest similar or previous searches to save
                    the user time: <a href="#free-solo">free solo</a>.
                </li>
            </ol>
            <Typography className="mb-16" component="div">It&#39;s meant to be an improved version of the &quot;react-select&quot; and &quot;downshift&quot; packages.</Typography>
            ...
            <Typography className="mb-16" component="div">Choose one of the 248 countries.</Typography>
            <Typography className="mb-16" component="div"><FuseExample
                className="my-24"
                iframe={false}
            ...
            <ol>
                <li>the &quot;value&quot; state with the <code>{`value`}</code>/<code>{`onChange`}</code> props combination. This state represents the value selected by the user, for instance when
                    pressing <kbd>Enter</kbd>.
                </li>
                <li>the &quot;input value&quot; state with the <code>{`inputValue`}</code>/<code>{`onInputChange`}</code> props combination. This state represents the value displayed in the textbox.
                </li>
            </ol>
            <blockquote>
            ...
            <Typography className="mb-16" component="div">If you intend to use this mode for a <a href="#combo-box">combo box</a> like experience (an enhanced version of a select element) we recommend
                setting:</Typography>
            <ul>
...
`}
            ...
            /></Typography>
            <Typography className="mb-16" component="div">Head to the <a href="#customized-autocomplete">Customized Autocomplete</a> section for a customization example with
                the <code>{`Autocomplete`}</code> component instead of the hook.</Typography>
       ...
       {/*     <Typography className="text-24 mt-32 mb-8" component="h3">Google Maps place</Typography>
            ...
            /></Typography>
            <Typography className="text-24 mt-32 mb-8" component="h3">Limit tags</Typography>
            <Typography className="mb-16" component="div">You can use the <code>{`limitTags`}</code> prop to limit the number of displayed options when not focused.</Typography>
            <Typography className="mb-16" component="div"><FuseExample
                className="my-24"
                iframe={false}
                component={require('app/main/documentation/material-ui-components/components/autocomplete/LimitTags.js').default}
            ...
            /></Typography>
            <Typography className="mb-16" component="div">Head to the <a href="#customized-hook">Customized hook</a> section for a customization example with the <code>{`useAutocomplete`}</code> hook
                instead of the component.</Typography>
            <Typography className="text-32 mt-32 mb-8" component="h2">Highlights</Typography>
            <Typography className="mb-16" component="div">The following demo relies on <a href="https://github.com/moroshko/autosuggest-highlight">autosuggest-highlight</a>, a small (1 kB) utility for
                highlighting text in autosuggest and autocomplete components.</Typography>
            <Typography className="mb-16" component="div"><FuseExample
                className="my-24"
                iframe={false}
...
`}
            ...
            <Typography className="text-24 mt-32 mb-8" component="h3"><code>{`createFilterOptions(config) => filterOptions`}</code></Typography>
            ...
            <ol>
                <li><code>{`config`}</code> (<em>Object</em> [optional]):
                    <ul>
                        ...
                        <li><code>{`config.limit`}</code> (<em>Number</em> [optional]): Default to null. Limit the number of suggested options to be shown. For example,
                            if <code>{`config.limit`}</code> is <code>{`100`}</code>, only the first <code>{`100`}</code> matching options are shown. It can be useful if a lot of options match and
                            virtualization wasn&#39;t set up.
                        </li>
                        <li><code>{`config.matchFrom`}</code> (<em>&#39;any&#39; | &#39;start&#39;</em> [optional]): Defaults to <code>{`'any'`}</code>.</li>
                        <li><code>{`config.stringify`}</code> (<em>Func</em> [optional]): Controls how an option is converted into a string so that it can be matched against the input text fragment.
                        </li>
            ...
            <Typography className="mb-16" component="div"><code>{`filterOptions`}</code>: the returned filter method can be provided directly to the <code>{`filterOptions`}</code> prop of
                the <code>{`Autocomplete`}</code> component, or the parameter of the same name for the hook.</Typography>
...
`}
            ...
            <Typography className="mb-16" component="div">For richer filtering mechanisms, like fuzzy matching, it&#39;s recommended to look at <a
                href="https://github.com/kentcdodds/match-sorter">match-sorter</a>. For instance:</Typography>
...
`}
            ...
            <Typography className="mb-16" component="div">Search within 10,000 randomly generated options. The list is virtualized thanks to <a
                href="https://github.com/bvaughn/react-window">react-window</a>.</Typography>
            <Typography className="mb-16" component="div"><FuseExample
                className="my-24"
                iframe={false}
            ...
            <Typography className="mb-16" component="div">By default, the component disable the <strong>autocomplete</strong> feature (remembering what the user has typed for a given field in a
                previous session) with the <code>{`autoComplete="off"`}</code> attribute.</Typography>
            <Typography className="mb-16" component="div">However, in addition to remembering past entered values, the browser might also propose <strong>autofill</strong> suggestions (saved login,
                address, or payment details).
                In the event you want the avoid autofill, you can try the following:</Typography>
            <ul>
                <li><Typography className="mb-16" component="div">Name the input without leaking any information the browser can use. e.g. <code>{`id="field1"`}</code> instead
                    of <code>{`id="country"`}</code>. If you leave the id empty, the component uses a random id.</Typography>
                </li>
...
`}
            ...
            <Typography className="mb-16" component="div">If you provide a custom <code>{`ListboxComponent`}</code> prop, you need to make sure that the intended scroll container has
                the <code>{`role`}</code> attribute set to <code>{`listbox`}</code>. This ensures the correct behavior of the scroll, for example when using the keyboard to navigate.</Typography>
