...
function Chat(props) {
	...
	const phone = useSelector(({ chatApp }) => chatApp.contacts.selectedPhone);
	...
	function onMessageSubmit(ev) {
		...
		dispatch(
			...
			setLoading(false)
			if (res.error) {
				enqueueSnackbar(`Can't send SMS to this phone!`, {
					variant: 'error',
					autoHideDuration: 3000
				});
			} else {
				enqueueSnackbar('Sent SMS successfully!', { variant: 'success', autoHideDuration: 2000 });
			}
		});
	}
							...
							{chat.dialog.map((item, i) => {
								...
								return (
									<div
										...
										className={clsx(
											classes.messageRow,
											'flex flex-col flex-grow-0 flex-shrink-0 items-start justify-end relative px-16 pb-32',
											{ me: item.who === user.id },
									...
									>
										...
										<div className="bubble flex relative items-center justify-center p-12 max-w-full shadow">
											...
											<Typography
												className="time flex absolute w-full text-11 mt-8 -mb-24 left-0 bottom-0 whitespace-nowrap"
												color="textSecondary"
											>
