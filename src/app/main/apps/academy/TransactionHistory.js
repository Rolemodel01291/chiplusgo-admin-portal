...
import { getClient, selectedClient } from './store/clientSlice';
...
function TransactionHistory() {
	...
	const clientData = useSelector(({ academyApp }) => academyApp.client.selectedClient);
	const selectedTransaction = useSelector(({ academyApp }) => academyApp.transaction.selectedTransaction);
	const transactiondata = useSelector(selectTransaction);

	const transactions = _.orderBy(transactiondata, 'Create_date', 'desc');
	...
	// useEffect(() => {
	// 	const tempTransactions = _.orderBy(transactiondata, ['Create_date'], ['desc']);
	// 	setState({...state, transactions: tempTransactions})
	// }, [transactiondata])

	useEffect(() => {
		dispatch(getTransaction(clientData.id)).then(() => {
							...
							<div className="flex w-full p-12">
								<div className="w-1/2 pr-12">
									...
									<div className="border-b-1 border-gray-400 py-12 flex justify-between">
										...
										<Typography> {state.selectedTransaction.ClientName?state.selectedTransaction.ClientName:state.selectedTransaction.clientData.Name}</Typography>
									</div>
